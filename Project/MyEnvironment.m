classdef MyEnvironment < rl.env.MATLABEnvironment
    ...
    PlotFrequency = 50; % Specifies how often to plot (every 50 episodes)
    end
    ...
    methods              
        ...
        function this = MyEnvironment()
            ...
            actionValues = -20:2:20;  % Example set of discrete actions
        ...
        function [Observation,Reward,IsDone,Info] = step(this,Action)
            ...
            if storeangle < 0
                storeangle = 2*pi + storeangle;
            end
            this.ThetaHistory = [this.ThetaHistory; storeangle];
...
% Ensure the plot figure and subplots are set up
            if isempty(this.PlotAxes) || any(~isgraphics(this.PlotAxes))
                ...
                this.PlotAxes(3) = subplot(3,1,3); % For theta vs time plot
            end
            ...
            plot(this.PlotAxes(3), this.TimeHistory, this.Actions, 'r-');
            title(this.PlotAxes(3), 'Torque vs. Time');
            xlabel(this.PlotAxes(3), 'Time (s)');
            ylabel(this.PlotAxes(3), 'Torque');
